...
import 'package:scholar_time_table_app/src/core/instance/assignments/schedule/domain/ScheduleAssignment.dart';
import 'package:scholar_time_table_app/src/core/instance/assignments/schedule/domain/ScheduleClassRoom.dart';
import 'package:scholar_time_table_app/src/core/instance/assignments/schedule/domain/ScheduleCourse.dart';
import 'package:scholar_time_table_app/src/core/instance/assignments/schedule/domain/ScheduleProblem.dart';
import 'package:scholar_time_table_app/src/core/instance/assignments/schedule/domain/ScheduleSubject.dart';
import 'package:scholar_time_table_app/src/core/instance/assignments/schedule/domain/ScheduleTeacher.dart';
...
void main(){
  group("Assignment Scheduler", () {
    ...
    test("Should true when check subject is assigned", () {
      // Arrange
      var subjects            = [ScheduleSubject("S1", [1, 1, 1]), ScheduleSubject("S2", [1, 1])];
      var teachers            = [ScheduleTeacher("T1", ["S1"]), ScheduleTeacher("T2", ["S2"])];
      var classRooms          = [ScheduleClassRoom("A1"), ScheduleClassRoom("A2")];
  ...
  group("Check Subject sessions restrictions", (){
    test("Should return true because all subject sessions is scheduled and in different days", (){
      ...
      var timeFrames          = ScheduleTimeFrameMother.withDaysAndPeriodPerTurn(2, 2);
      var assignmentScheduler = ScheduleAssignment.empty(new ScheduleProblem(subjects, teachers, classRooms, courses, timeFrames));
  ...
  group("Check teacher schedule gaps", (){
    test("Should return 0 because not exist gaps", (){
      // Arrange
      var subjects            = [ScheduleSubject("S1", [1]), ScheduleSubject("S2", [1]), ScheduleSubject("S3", [1])];
      ...
      // Act
      var result = assignmentScheduler.assign(0, 0, 2);
      result     = result.assign(1, 0, 3);

      // Assert
      expect(result.countTeacherGaps(), isZero);
    });
    test("Should return 2 because exist gaps", (){
      // Arrange
      var subjects            = [ScheduleSubject("S1", [1]), ScheduleSubject("S2", [1]), ScheduleSubject("S3", [1])];
      var teachers            = [ScheduleTeacher("T1", ["S1", "S2"]), ScheduleTeacher("T2", ["S3"])];
      var classRooms          = [ScheduleClassRoom("A1"), ScheduleClassRoom("A2")];
